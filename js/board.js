...
searchInput.placeholder='板検索'; searchInput.style.width='200px'
searchInput.addEventListener('keyup',()=>{searchQuery=searchInput.value.trim(); loadBoards(true)})
...
async function loadBoards(reset=false){
  if(loading) return
  loading=true
  if(reset) currentPage=0; if(reset) div.innerHTML=''
...
// 無限スクロール
window.addEventListener('scroll',()=>{if(window.innerHeight+window.scrollY>=document.body.offsetHeight-50){loadBoards()}})
